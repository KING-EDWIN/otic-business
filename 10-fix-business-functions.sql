-- Fix Business Management Functions
-- This file drops existing functions and recreates them with correct types

-- Drop existing functions first
DROP FUNCTION IF EXISTS can_create_business(UUID);
DROP FUNCTION IF EXISTS get_user_businesses(UUID);
DROP FUNCTION IF EXISTS create_business(VARCHAR, TEXT, VARCHAR, VARCHAR, UUID);
DROP FUNCTION IF EXISTS switch_business_context(UUID, UUID);
DROP FUNCTION IF EXISTS get_current_business_context(UUID);
...
-- Function to get user's businesses (simplified return type)
