-- Fix the delete_user_completely function to properly handle auth.users deletion
-- The issue is that auth.users deletion requires special permissions
...
BEGIN
    ...
    -- Try to delete from auth.users - this might fail due to permissions
    -- but we'll continue anyway since the main data is deleted
    BEGIN
        DELETE FROM auth.users WHERE id = user_id_var;
        RAISE NOTICE 'Auth user deleted successfully: %', user_email;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'Auth user deletion failed (this is expected): % - %', user_email, SQLERRM;
    END;
