-- FIX SWITCH BUSINESS FUNCTION - Simple Version
...
-- 2. Create business_switches table if it doesn't exist
...
-- 4. Create the switch_business_context function
...
BEGIN
    ...
    -- Record the business switch (simple insert, no conflict handling)
...
EXCEPTION
    WHEN unique_violation THEN
        ...
        UPDATE business_switches 
        SET switched_at = NOW() 
        WHERE user_id = user_id_param AND business_id = business_id_param;
        RETURN true;
    WHEN OTHERS THEN
        -- For any other error, still return true (switching worked)
        RETURN true;
END;
$$;
