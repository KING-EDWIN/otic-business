-- Complete Invoice Tables Setup for Otic Business
-- This script ensures all invoice-related tables are properly created and configured
...
DROP POLICY IF EXISTS "Business members can view business invoices" ON invoices;
CREATE POLICY "Business members can view business invoices" ON invoices
  FOR ALL USING (
    business_id IN (
...
DROP POLICY IF EXISTS "Business members can view business invoice items" ON invoice_items;
CREATE POLICY "Business members can view business invoice items" ON invoice_items
  FOR ALL USING (
    invoice_id IN (
      SELECT i.id FROM invoices i
      JOIN business_memberships bm ON i.business_id = bm.business_id
...
DROP POLICY IF EXISTS "Business members can view business invoice templates" ON invoice_templates;
CREATE POLICY "Business members can view business invoice templates" ON invoice_templates
  FOR ALL USING (
    business_id IN (
      SELECT business_id FROM business_memberships 
      WHERE user_id = auth.uid() AND status = 'active'
