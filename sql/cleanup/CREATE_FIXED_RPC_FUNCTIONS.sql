-- Create RPC functions with proper foreign key handling
...
-- 1. Create create_branch_sale function (with optional cashier_id)
CREATE OR REPLACE FUNCTION create_branch_sale(
    branch_id_param UUID,
    payment_method_param TEXT,
    cashier_id_param UUID DEFAULT NULL
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    INSERT INTO branch_sales (
        ...
        branch_id_param,
        cashier_id_param, -- Can be NULL if no valid cashier
        NOW(),
...
-- Test the functions with NULL cashier_id
SELECT 'Testing create_branch_sale with NULL cashier...' as status;
SELECT create_branch_sale(
    '066efb80-2a71-41c5-b704-4d9574b5d5bf'::UUID,
    'cash',
    NULL -- No cashier_id to avoid foreign key constraint
) as sale_id;

-- Test with a valid user ID (if you have one)
-- SELECT create_branch_sale(
--     '066efb80-2a71-41c5-b704-4d9574b5d5bf'::UUID,
--     'cash',
...
SELECT 'All RPC functions created successfully with proper foreign key handling!' as status;
