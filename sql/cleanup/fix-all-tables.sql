-- =====================================================
-- FIX ALL OTIC VISION TABLES
-- =====================================================
-- This script ensures all tables have the correct structure

-- 1. Drop and recreate personalised_visual_bank table
...
-- 2. Drop and recreate token_similarity_log table
...
-- 3. Drop and recreate color_analysis_cache table
...
-- 4. Drop and recreate recognition_sessions table
...
DROP POLICY IF EXISTS "Users can view own products" ON personalised_visual_bank;
DROP POLICY IF EXISTS "Users can insert own products" ON personalised_visual_bank;
DROP POLICY IF EXISTS "Users can update own products" ON personalised_visual_bank;
DROP POLICY IF EXISTS "Users can delete own products" ON personalised_visual_bank;
...
DROP POLICY IF EXISTS "Users can view own similarity logs" ON token_similarity_log;
DROP POLICY IF EXISTS "Users can insert own similarity logs" ON token_similarity_log;
...
DROP POLICY IF EXISTS "Anyone can read color cache" ON color_analysis_cache;
DROP POLICY IF EXISTS "Authenticated users can insert color cache" ON color_analysis_cache;
...
DROP POLICY IF EXISTS "Users can view own sessions" ON recognition_sessions;
DROP POLICY IF EXISTS "Users can insert own sessions" ON recognition_sessions;
DROP POLICY IF EXISTS "Users can update own sessions" ON recognition_sessions;
...
-- Success message
SELECT 'All OTIC Vision tables fixed successfully!' as status;
