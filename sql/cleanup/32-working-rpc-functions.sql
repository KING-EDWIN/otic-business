...
-- Create can_create_business function with correct tier values
...
AS $$
  SELECT CASE 
    ...
    WHEN up.tier = 'start_smart' AND (SELECT COUNT(*) FROM business_memberships WHERE user_id = user_id_param AND role = 'owner') >= 1 THEN FALSE
    WHEN up.tier = 'grow_intelligence' AND (SELECT COUNT(*) FROM business_memberships WHERE user_id = user_id_param AND role = 'owner') >= 5 THEN FALSE
    WHEN up.tier = 'enterprise_advantage' THEN TRUE
    WHEN up.tier = 'basic' AND (SELECT COUNT(*) FROM business_memberships WHERE user_id = user_id_param AND role = 'owner') >= 1 THEN FALSE
    WHEN up.tier = 'standard' AND (SELECT COUNT(*) FROM business_memberships WHERE user_id = user_id_param AND role = 'owner') >= 3 THEN FALSE
