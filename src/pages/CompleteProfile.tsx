  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      if (user) {
        ...
        // Handle Google sign-up users
        const locationState = location.state as any
        if (locationState?.isGoogleSignup && locationState?.googleUser) {
          const googleUser = locationState.googleUser
          setFormData(prev => ({
            ...prev,
            businessName: googleUser.user_metadata?.full_name || googleUser.email?.split('@')[0] || '',
            tier: locationState.userType === 'individual' ? 'free_trial' : 'free_trial'
          }))
        } else {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-gray-600">
            {(location.state as any)?.isGoogleSignup 
              ? "Welcome! Your Google account is linked. Just add your business details to get started."
              : "Complete your business profile to get started with Otic Business."
            }
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Linked Google Account - Only show for Google sign-up users */}
            {(location.state as any)?.isGoogleSignup && (
