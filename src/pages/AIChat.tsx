...
const AIChat: React.FC = () => {
  ...
  const getBusinessContext = async () => {
    try {
      ...
      console.log('🤖 AI Chat: Fetching business context for user:', user.id)
      ...
      return {
        ...
        totalRevenue: businessData.totalRevenue,
        totalProducts: businessData.totalProducts,
        growthRate: businessData.salesGrowth,
        lowStockCount: businessData.lowStockItems.length,
        recentSales: businessData.recentSales.slice(0, 5).map(sale => ({
          date: new Date(sale.created_at).toISOString().split('T')[0],
          amount: sale.total,
          method: sale.payment_method || 'cash'
        })),
        topProducts: businessData.topProducts.slice(0, 5)
      }
    } catch (error) {
...
What would you like to know about your business?`,
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <header className="bg-white shadow-sm py-3 lg:py-4 flex-shrink-0">
          ...
          <div className="flex flex-col space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
            <div className="flex flex-col space-y-2 lg:flex-row lg:items-center lg:space-x-3 lg:space-y-0">
              <Button
                ...
                className="text-[#040458] hover:text-[#faa51a] hover:bg-[#faa51a]/10 w-fit"
              >
              ...
              <div className="flex items-center space-x-2">
                <Brain className="h-6 w-6 lg:h-8 lg:w-8 text-[#040458]" />
                <h1 className="text-xl lg:text-3xl font-bold text-[#040458]">AI Business Assistant</h1>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-h-0">
        <div className="flex-1 flex flex-col lg:flex-row gap-3 lg:gap-4 p-3 lg:p-4 max-w-7xl mx-auto w-full">
          {/* Quick Questions Sidebar */}
          <div className="w-full lg:w-80 flex-shrink-0">
            <Card className="h-full">
              <CardHeader className="pb-3 lg:pb-4">
                <CardTitle className="flex items-center text-[#040458] text-base lg:text-lg">
                  <MessageSquareText className="h-4 w-4 lg:h-5 lg:w-5 mr-2 text-[#faa51a]" />
                  Quick Questions
                </CardTitle>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <Card className="flex-1 flex flex-col min-h-0">
              <CardHeader className="pb-3 lg:pb-4 flex-shrink-0">
                <CardTitle className="flex items-center text-[#040458] text-lg lg:text-xl">
                  <Bot className="h-5 w-5 lg:h-6 lg:w-6 mr-2 lg:mr-3 text-[#faa51a]" />
                  ...
                  <ScrollArea className="h-full" ref={scrollAreaRef}>
                    <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
                      {messages.map((message) => (
                        ...
                        >
                          <div
                            className={`max-w-[90%] lg:max-w-[85%] rounded-lg p-3 lg:p-4 ${
                              message.type === 'user'
                          ...
                          >
                            <div className="flex items-start space-x-2 lg:space-x-3">
                              {message.type === 'ai' ? (
                                <Bot className="h-4 w-4 lg:h-5 lg:w-5 mt-1 text-[#faa51a] flex-shrink-0" />
                              ) : (
                                <User className="h-4 w-4 lg:h-5 lg:w-5 mt-1 flex-shrink-0" />
                              )}
                              <div className="flex-1 min-w-0">
                                <p className="text-xs lg:text-sm whitespace-pre-wrap break-words leading-relaxed">
                                  {message.content}
                                </p>
                ...
                <div className="p-4 lg:p-6 border-t border-gray-200 flex-shrink-0">
                  <div className="flex space-x-2 lg:space-x-3">
                    <Input
                      ...
                      className="flex-1 text-sm lg:text-base h-10 lg:h-12"
                      disabled={isLoading}
                    />
                    <Button
                      onClick={() => handleSendMessage()}
                      disabled={!inputMessage.trim() || isLoading}
                      className="bg-[#faa51a] hover:bg-[#040458] text-white px-4 lg:px-6 h-10 lg:h-12"
                    >
