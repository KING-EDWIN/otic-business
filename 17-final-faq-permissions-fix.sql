...
-- First, check if categories exist, if not insert them
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM faq_categories WHERE name = 'Getting Started') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM faq_questions WHERE question = 'How do I create my account?') THEN
