-- Fix coupons table structure and create missing functions
-- This script handles both new table creation and existing table updates

-- First, check if coupons table exists and add missing columns
DO $$
BEGIN
    -- Create table if it doesn't exist
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'coupons') THEN
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'tier') THEN
            ALTER TABLE coupons ADD COLUMN tier VARCHAR(50);
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'description') THEN
            ALTER TABLE coupons ADD COLUMN description TEXT;
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'is_active') THEN
            ALTER TABLE coupons ADD COLUMN is_active BOOLEAN DEFAULT true;
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'is_used') THEN
            ALTER TABLE coupons ADD COLUMN is_used BOOLEAN DEFAULT false;
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'used_by') THEN
            ALTER TABLE coupons ADD COLUMN used_by UUID REFERENCES auth.users(id);
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'used_at') THEN
            ALTER TABLE coupons ADD COLUMN used_at TIMESTAMP WITH TIME ZONE;
        END IF;
        
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'created_by') THEN
            ALTER TABLE coupons ADD COLUMN created_by UUID REFERENCES auth.users(id);
        END IF;
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'coupons' AND column_name = 'metadata') THEN
            ALTER TABLE coupons ADD COLUMN metadata JSONB DEFAULT '{}'::jsonb;
        END IF;
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Allow authenticated users to read active coupons" ON coupons;
...
-- Function to generate random 5-digit coupon code (FIXED VERSION)
...
BEGIN
    LOOP
        ...
        coupon_code := LPAD(FLOOR(RANDOM() * 100000)::TEXT, 5, '0');
...
-- Function to create a coupon (FIXED VERSION)
...
-- Insert some sample coupons for testing (only if they don't exist)
...
-- Verify the setup
SELECT 'Coupons table setup completed successfully!' as status;
