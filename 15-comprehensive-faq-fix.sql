-- Comprehensive FAQ System Fix
-- This file completely fixes the FAQ system access issues

-- First, disable RLS temporarily to ensure we can fix the policies
...
DROP POLICY IF EXISTS "FAQ categories are publicly viewable" ON faq_categories;
DROP POLICY IF EXISTS "FAQ questions are publicly viewable" ON faq_questions;
DROP POLICY IF EXISTS "Authenticated users can insert search logs" ON faq_search_logs;
...
CREATE POLICY "Allow all operations on faq_categories" ON faq_categories
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all operations on faq_questions" ON faq_questions
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all operations on faq_search_logs" ON faq_search_logs
    FOR ALL USING (true) WITH CHECK (true);

-- Ensure the tables exist and have data
